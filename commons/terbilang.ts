...
const numberToText = (index: any) => {
  return numbers[index] || "";
};

const terbilang = (angka: any) => {
...
const terbilangSatuSatu = (angka: any) => {
  return angka
    .split("")
    .map((angka: any) => (angka == 0 ? "nol" : numberToText(angka)))
    .join(" ");
};

const terbilangNumber = (target: any, settings = { decimal: "." }) => {
...
export default terbilangNumber;
